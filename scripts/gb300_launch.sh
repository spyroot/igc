...
export HF_HOME HF_HUB_CACHE HUGGINGFACE_HUB_CACHE IGC_NCCL_MNNVL NCCL_MNNVL_ENABLE NCCL_CUMEM_ENABLE
...
docker run --rm --gpus all --ipc=host --ulimit memlock=-1 --ulimit stack=67108864 \
    ...
    -e IGC_NCCL_MNNVL -e NCCL_MNNVL_ENABLE -e NCCL_CUMEM_ENABLE \
    "$IGC_IMAGE" bash -lc '
        set -uo pipefail
        cd /workspace/igc
        _igc_nccl_mnnvl="${IGC_NCCL_MNNVL:-${NCCL_MNNVL_ENABLE:-0}}"
        [ -f .internal/hf.env ] && { set -a; . .internal/hf.env; set +a; }
        export HF_HOME="${HF_HOME:-/models/hf-cache}"
        export HF_HUB_CACHE="${HF_HUB_CACHE:-${HF_HOME}/hub}"
        export HUGGINGFACE_HUB_CACHE="${HUGGINGFACE_HUB_CACHE:-${HF_HOME}/hub}"
        export NCCL_MNNVL_ENABLE="${_igc_nccl_mnnvl}"
