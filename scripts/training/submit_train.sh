...
# then submits scripts/training/train_igc.sbatch for the chosen stage. See GPU_ACCESS.md.
#
#   ./scripts/training/submit_train.sh m1                 # smoke: gpt2 state encoder -> wandb
#   IGC_MODEL=<hf-id> IGC_USE_PEFT=1 EPOCHS=3 ./scripts/training/submit_train.sh m1
#   ./scripts/training/submit_train.sh m6 -w gb300-poc1-slot7   # pin to a node where data is staged
#
...
SBATCH_FILE="${HERE}/scripts/training/train_igc.sbatch"
...
if [[ "${IGC_SKIP_PREFLIGHT:-0}" != "1" ]]; then
    "${HERE}/scripts/nv72/preflight_nv72.sh" || {
