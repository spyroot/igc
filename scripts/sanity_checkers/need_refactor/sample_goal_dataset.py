#!/usr/bin/env python3
"""Sample and summarize a parked legacy GoalExtractor / GoalEncoder dataset.

PARKED: this helper is retained for Phase 1/2/3 refactor reference only. It is
not a current acceptance checker.

This inspection helper reads the JSONL artifacts produced by
``scripts/sanity_checkers/need_refactor/build_goal_dataset.py`` or ``scripts/sanity_checkers/need_refactor/build_goal_dataset_lab.sh`` and
...
# Running this parked helper from ``scripts/sanity_checkers/need_refactor`` puts
# that directory on sys.path; add the repo root so ``import igc`` still works.
sys.path.insert(
    0,
