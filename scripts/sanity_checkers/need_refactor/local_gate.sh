#!/usr/bin/env bash
# local_gate.sh - parked legacy laptop/offline gate wrapper.
#
# PARKED: current IGC validation must run through Internal GitLab/Kubernetes or
# an approved remote GB300/NV72 surface, not on the operator laptop. Keep this
# file only as refactor reference until a focused PR removes or rewrites it.
#
...
#
# Legacy usage:
#   bash scripts/sanity_checkers/need_refactor/local_gate.sh                     # full offline gate
#   bash scripts/sanity_checkers/need_refactor/local_gate.sh tests/ds -q         # extra pytest args pass through
#   IGC_GATE_PYTHON=/path/to/python bash scripts/sanity_checkers/need_refactor/local_gate.sh
#
# Env knobs:
...
#
# Historical use: serialized local gate entrypoint for older coordination
# passes. Current agents must not use it as the IGC validation path.
#
# Author:
