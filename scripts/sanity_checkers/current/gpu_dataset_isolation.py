...
Run one case (under torchrun):
    DS_SIZE=97 BATCH=8 DROP_LAST=0 SAMPLER=custom \
      torchrun --nproc_per_node=4 scripts/sanity_checkers/current/gpu_dataset_isolation.py

Env: DS_SIZE (dataset length), BATCH, DROP_LAST (0/1), SAMPLER (none|custom), EPOCHS (default 2).
Used by: scripts/sanity_checkers/current/gpu_dataset_isolation.sh (sweeps the edge-case matrix with per-case timeouts).
