...
Used by: scripts/nv72/gb300_sanity_check.sh with SANITY_SCRIPT=scripts/sanity_checkers/current/dataset_sanity.py.
...
# torchrun runs this from the script directory, so that directory (not the repo
# root) is on sys.path; add the repo root so `import igc` resolves inside the
# container.
sys.path.insert(
    0,
