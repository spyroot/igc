...
#   IGC_RUNG=smoke1 scripts/nv72/gb300_launch.sh   # 1 GPU, ~20 steps, W&B off — is the path alive?
#   IGC_RUNG=smoke4 scripts/nv72/gb300_launch.sh   # 4 GPU DDP, ~20 steps, W&B off — does DDP wire up?
#   IGC_RUNG=run4   scripts/nv72/gb300_launch.sh   # 4 GPU DDP, real short run -> W&B
#   IGC_RUNG=fsdp4  scripts/nv72/gb300_launch.sh   # 4 GPU FSDP2, real short run -> W&B (only if it must shard)
...
#   IGC_DRY_RUN=1 IGC_RUNG=run4 IGC_MODEL=gpt2 scripts/nv72/gb300_launch.sh
#
...
#   nohup setsid bash scripts/nv72/gb300_launch.sh > ~/igc-run.log 2>&1 </dev/null & exit 0
#
# Used by: run by hand on a free reserved slot (GPU_ACCESS.md §3), or after the 2-node
# DDP sanity gate (scripts/nv72/gb300_sanity_check.sh) proves the fabric. Not imported by Python.
...
# Model / data / output
IGC_STAGE="${IGC_STAGE:-m1}"                                # LLM curriculum stage -> --train/--llm (m1=latent|m2=encoder|m3=goal|m3p=parameter). RL/combined (m6/all) live in scripts/training/train_igc.sbatch
...
# resolve the LLM curriculum stage -> the real --train/--llm flags (scripts/training/train_igc.sbatch is the
...
case "$IGC_STAGE" in
    ...
    m6|agent|rl|all)  blocker "IGC_STAGE='${IGC_STAGE}' (RL/combined) is not wired here — use scripts/training/train_igc.sbatch; this launcher covers the LLM stages m1|m2|m3|m3p" ;;
