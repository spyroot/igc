...
igc has offline CPU hot-path benchmarks (``scripts/profilers/bench_hot_paths.py`` — the RL decision path)
and a distributed dataset-isolation harness (``scripts/sanity_checkers/current/gpu_dataset_isolation.py`` — the epoch-2
...
operator tool. See ``scripts/profilers/gpu_train_profile.sh`` for the 4- and 8-GPU sweep recipe.

Run (one process, smoke): ``python scripts/profilers/gpu_train_profile.py --steps 5 --warmup 2``
Run (4 GPU, faithful):    ``accelerate launch --multi_gpu --num_processes 4 \
                              scripts/profilers/gpu_train_profile.py --model_type gpt2 --batch_size 8 \
