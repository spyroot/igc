...
#
# Mirrors scripts/sanity_checkers/current/gpu_dataset_isolation.sh: one profiled configuration per accelerate launch, each
...
#   scripts/profilers/gpu_train_profile.sh                       # default sweep: {4,8} GPU x {4,8} batch, bf16
#   GPUS="4" BATCHES="8 16" MODEL=gpt2 scripts/profilers/gpu_train_profile.sh
#
# Env knobs:
...
for g in $GPUS; do
    ...
    for b in $BATCHES; do
        ...
        accelerate launch --multi_gpu --num_processes "$g" --num_machines 1 --mixed_precision "$PRECISION" \
            "${here}/scripts/profilers/gpu_train_profile.py" \
