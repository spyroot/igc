...
def test_cli_mock_mode_writes_rendered_rows_and_metrics(
    ...
    assert "calls" in rows[0]["inference"]
    assert "ordered_goals" not in rows[0]["inference"]
    assert "target_calls" not in rows[0]["inference"]
    assert rows[0]["inference"]["calls"] == rows[0]["y_true"]["calls"]
    patch_call = rows[1]["inference"]["calls"][0]
    assert patch_call["http_method"] == "PATCH"
    ...
    assert metrics[_metric("eval", "http_method_exact_match_rate")] == 1.0
...
def test_cli_extra_predicted_call_fails_unordered_call_set_match(tmp_path: Path) -> None:
    """Extra predicted calls are counted as unordered call-set failures."""
    script = _load_script()
...
def test_cli_invalid_method_returns_nonzero_after_writing_metrics(tmp_path: Path) -> None:
    ...
    assert "not allowed" in rows_out[0]["evaluation"]["reason"]
