...
avoid duplicate OpenMP loader failures and keep CPU checks single-threaded. `TRANSFORMERS_OFFLINE`
and `HF_DATASETS_OFFLINE`, local shell variables honored by HuggingFace Transformers and Datasets,
keep the smoke gate from attempting model or dataset downloads.
