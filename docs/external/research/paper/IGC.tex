...
The state-representation learner sees the observation projection:
...
\subsection*{Why RL Phase 0 discovery pretraining is valuable}
...
So the exploration task has exact labels, exact transition traces, an exact terminal condition, an exact maximum return, and an exact expert solution. It runs after the Phase~1/2/3 supervised stack and before task-conditioned RL. It does not add a graph dependency to Phase~1/2/3.

An optional graph encoder can be trained on the same discovery traces the policy uses, producing node embeddings and a graph-pooled state candidate. The non-graph StateEncoder remains the v1 baseline. The graph variant is admitted only if later state-sufficiency and RL gates outperform that baseline; neither JSON graph construction nor a GNN is a public dependency.

\subsection*{Phase 2/3 outputs attach human intent to the same process}

During RL Phase~0 discovery pretraining, the goal is to visit all reachable nodes. During subsequent task-conditioned RL, the already-promoted Phase~2/3 outputs provide the compiled operation goal:
...
The same version-pinned StateEncoder selected for an experiment represents the state. The same policy operates over discovered resources. The only change is the goal conditioning.
